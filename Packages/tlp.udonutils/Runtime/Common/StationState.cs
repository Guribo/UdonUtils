...
namespace TLP.UdonUtils.Runtime.Common
{
    ...
    public class StationState : TlpBaseBehaviour
    {
        ...
        [PublicAPI]
        public new const int ExecutionOrder = Chair.ExecutionOrder + 1;
        #endregion
        ...
        public bool ValidateNotOnStationGameObject(VRCStation station) {
            ...
            Error(
                    $"{nameof(StationState)} is on the same GameObject as the station: {Station.GetComponentPathInScene()}");
            return false;
        }
