...
namespace TLP.UdonUtils.Runtime.Player
{
    ...
    public class PlayerList : TlpBaseBehaviour
    {
        public int[] players = new int[0];
        public readonly DataList PlayerIdList = new DataList();
        ...
        public bool AddPlayer(VRCPlayerApi player)
        {
            ...
            // for (int i = 0; i < PlayerIdList.Count; i++) // TODO use 2 lists, when consolidating copy the values from one into the other
            // {
            //     int playerId = (int)PlayerIdList[i];
            //     if (playerId.IsValidPlayer(out var resultPlayer))
            //     {
            //         continue;
            //     }
            //
            ...
            bool playerNotInList = Array.BinarySearch(players, player.playerId) < 0;
            if (!Assert(playerNotInList, $"Player {player.playerId} already in list", this))
            {
                ResizePlayerArray(validPlayers);
                return false;
            }
