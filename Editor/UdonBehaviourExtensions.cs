...
namespace TLP.UdonUtils.Editor
{
    ...
    public static class UdonBehaviourExtensions
    {
        ...
        /// <exception cref="ArgumentException">if udonBehaviour is invalid</exception>
        /// <exception cref="Exception">e.g. if the symbolName doesn't exist</exception>
        public static void SetInspectorVariable<T>(this UdonBehaviour udonBehaviour, string symbolName, T newValue)
        {
            if (!udonBehaviour)
            {
                throw new ArgumentException("Invalid UdonBehaviour");
            ...
            {
                throw new ArgumentException("Invalid symbolName");
            }

            if (Application.IsPlaying(udonBehaviour))
            {
                throw new InvalidOperationException("Only edit mode is supported");
            }
            
            // TODO investigate if this is the new approach
            // var so = new SerializedObject(udonBehaviour);
            // // you can Shift+Right Click on property names in the Inspector to see their paths
            // var serializedProperty = so.FindProperty("publicVariables");
            // Debug.Log(serializedProperty.propertyType.ToString());
            // so.ApplyModifiedProperties();

            var publicVariables = udonBehaviour.publicVariables;
            if (publicVariables == null)
            {
                throw new WarningException("UdonBehaviour has no public Variables");
            }

            Undo.RecordObject(udonBehaviour, "Modify variable");
            if (!publicVariables.TrySetVariableValue<T>(symbolName, newValue))
            {
                Debug.LogWarning($"{nameof(IUdonVariableTable.TrySetVariableValue)} failed");
                if (!publicVariables.TryAddVariable(CreateUdonVariable(symbolName, newValue, typeof(T))))
                {
                    throw new Exception($"Failed to add public variable '{symbolName}' value");
                }
            }
            ...
            if (foundValue
                ...
                && (variableType == typeof(GameObject)
                    ...
                    || variableType == typeof(UdonSharpBehaviour)
                    || variableType == typeof(Transform)))
            {
                Debug.Log($"Removing variable {symbolName}");
